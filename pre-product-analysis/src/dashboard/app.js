...
function buildProductIdea() {
  ...
  return {
    source: {
      ...
      source_endpoints: ["seller_dashboard_input", "shopee.search.item"],
    ...
    data_quality: {
      ...
      warnings: [
        "Connect Shopee Open API credentials before using these signals for official reporting.",
...
function renderComparisonInsights(report) {
  ...
  document.querySelector("#demandInsight").textContent = competitors.length
    ...
    : "No close competitors in the current comparison set, so demand confidence is low.";
  ...
  document.querySelector("#competitionInsight").textContent = strongest
    ...
    : "No direct competitor found in this category.";
  ...
  document.querySelector("#priceComparisonText").textContent = avgPrice
    ? `${priceGap >= 0 ? "Premium" : "Value"} position vs Shopee SG competitors.`
  ...
  document.querySelector("#reviewPressureText").textContent = avgReviews > 1000
    ...
    : avgReviews > 300
      ...
      : "Review pressure looks manageable in this comparison set.";
